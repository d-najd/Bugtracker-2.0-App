...
class UserManagementScreenModel(
...
) : BugtrackerStateScreenModel<UserManagementScreenState>(context,
    ...
    fun invertAuthority(userAuthority: UserAuthority) {
        if((mutableState.value as UserManagementScreenState.Success).authorities.contains(userAuthority)) {
    ...
    fun createAuthority(userAuthority: UserAuthority) {
        coroutineScope.launchIO {
            val authorities = (mutableState.value as UserManagementScreenState.Success).authorities.toMutableList()
